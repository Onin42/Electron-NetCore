...
function getCurrentSHA (author) {
  return new Promise((resolve, reject) => {
    ...
    get({
      host: 'api.github.com',
      path: `/repos/Onin42/Electron-NetCore/commits${isBranch ? '?sha=' + process.argv[2].split('#')[1] : ''}`,
      headers: {
        'User-Agent': author
      }
    }, res => {
...
function appendSHALink (sha, destDirName) {
  ...
  md = md.replace(
    ' using',
    `@[${sha.substring(0, 7)}](https://github.com/Onin42/Electron-NetCore/tree/${sha}) using`
  )
...
module.exports = {
  prompts: {
    ...
    description: {
      ...
      default: 'An electron-netcore project'
    },
    // plugins: {
    //   type: 'checkbox',
  ...
  complete (data) {
    getCurrentSHA(data.author).then(sha => {
      ...
      console.log([
        '\n---',
        '',
        'All set. Welcome to your new electron-netcore project!',
        // '',
        // 'Make sure to check out the documentation for this boilerplate at',
        // '\x1b[33mhttps://simulatedgreg.gitbooks.io/electron-netcore/content/\x1b[0m.',
        '',
